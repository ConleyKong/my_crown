...
class BaseModelMetaclass(type):
    ...
    def __new__(cls, name, bases, attrs):
        if not bases:
            return super(BaseModelMetaclass, cls).__new__(cls, name, bases, attrs)
        ...
        cls = super(BaseModelMetaclass, cls).__new__(cls, name, bases, attrs)
        ...
        cls._meta.db_table = "%s.%s" % (cls._meta.database.db_name, cls._meta.db_table)
...
class Model(metaclass=BaseModelMetaclass):
...
class SuperModel(metaclass=BaseModelMetaclass):
    def __init__(self, *args, **kwargs):
        self._data = self._meta.get_default_dict()
        self._obj_cache = {} # cache of related objects
        for k, v in kwargs.items():
